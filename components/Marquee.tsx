/**
 * Bandeau de mots-clés défilant en continu, séparés par un losange doré.
 * L'animation est purement CSS (voir .marquee dans globals.css) : pause au
 * survol, bords estompés, et arrêt automatique en mouvement réduit.
 */
export default function Marquee({
  items,
  className = '',
}: {
  items: string[];
  className?: string;
}) {
  const loop = [...items, ...items];

  return (
    <div className={`marquee ${className}`}>
      <div className="marquee-track">
        {loop.map((item, i) => (
          <span
            key={i}
            className="flex items-center whitespace-nowrap"
            aria-hidden={i >= items.length ? 'true' : undefined}
          >
            <span className="px-8 text-sm font-medium uppercase tracking-[0.2em]">
              {item}
            </span>
            <svg
              width="44"
              height="44"
              viewBox="0 0 270 270"
              fill="none"
              aria-hidden="true"
              className="shrink-0"
            >
              <path fill="rgba(245,240,232,0.50)" d="M176.41,125.16c-.31,6.02-.94,18.5-7.11,32.18-5.68,12.63-13.43,20.41-15.54,22.46-8.51,8.28-17.3,12.63-22.8,14.85-5.59-3.16-9.1-6.52-11.03-9.93-.63-1.1-1.09-2.21-1.4-3.33v-.03s-.02-.02-.02-.03c-.33-1.19-.49-2.39-.51-3.58-.15-12.23,15.27-23.99,22.71-28.99,10.94-7.32,19.24-15.79,19.24-15.79-4.95,11.03-16.37,17.31-27.79,30.27-11.42,12.94-1.19,22.62-1.19,22.62,6.38-2.97,12.14-6.95,17.19-11.84,2.56-2.51,9.07-9.45,13.81-20,5.53-12.29,6.13-23.75,6.41-29.27.03-.58.06-1.21.07-1.85,2.98-3.04,5.74-6.17,8.14-9.38.02.3.02.58.02.88,0,.44.03,6.42-.19,10.75Z"/>
              <path fill="rgba(245,240,232,0.50)" d="M147.01,91.04c-.06.09-.1.19-.17.31-.03.05-.05.09-.06.13-.54,1.01-1.45,2.65-2.82,4.52-.19.25-.4.54-.61.8-.27.33-.54.66-.82,1-4.06-2.07-7.92-4.52-11.57-7.28-3.85,2.92-7.96,5.49-12.29,7.63-7.9,3.94-16.3,6.44-25.08,7.51-.12,2.92-.19,5.85-.22,8.77,0,1.67,0,6.77.18,10.3.1,2.22.27,5.41.79,9.23-2.86,3.39-4.98,7.11-5.98,11.29-2.25-8.65-2.63-15.94-2.85-20.12-.22-4.34-.19-10.32-.19-10.75.05-6.67.37-12.23.67-16.21,6.56-.19,17.39-1.42,29.09-7.23,6.9-3.44,12.15-7.5,15.88-10.91,3.73,3.41,8.98,7.47,15.88,10.91.06.03.1.06.16.08Z"/>
              <path fill="rgba(199,161,90,0.55)" d="M184.68,106.91l-5.29-6.4c-.6,1.22-2.25,4.17-2.99,5.38-2.15,3.53-4.82,7.01-7.84,10.38-16.73,18.7-44.12,34.77-52.2,45.51-2.77,3.68-3.97,7.28-4.22,10.54-.66,8.26,4.61,14.42,4.92,14.78-.37-.25-.73-.52-1.09-.78-8.47-6.29-12.27-12.51-12.93-18.5-.09-.85-.12-1.67-.09-2.49v-.07c.02-.42.05-.83.09-1.24.02-.21.05-.4.07-.61.05-.43.12-.86.22-1.3.05-.22.09-.45.15-.67.03-.15.07-.31.12-.46,2.99-9.97,35.37-35.23,35.37-35.23,5.72-4.42,13.64-11.27,22.02-21.32,2.84-3.55,4.28-5.79,5.07-7.2.18-.32,1.65-2.97,1.65-2.97h0l-9.08-.98,25.99-17.92.05,31.56Z"/>
              <path fill="rgba(245,240,232,0.50)" d="M151.02,92.97c-.58,2.73-1.37,5.28-2.3,7.66-8.29,21.41-28.67,29.84-40.99,41.48-4.14,3.91-6.55,7.92-7.9,11.62-1.73,4.73-1.71,8.93-1.36,11.75.25,2.07.7,3.4.78,3.61-.24-.33-.46-.66-.67-.98-.09-.12-.18-.25-.25-.37-.39-.6-.78-1.21-1.16-1.83-.21-.33-.4-.66-.58-.97-5.87-10.23-5.43-18.16-1.18-24.9v-.02c5.25-8.31,16.22-14.82,28.18-21.64,12.27-6.98,19.27-13.78,23.13-18.59,3.01-3.76,4.1-6.31,4.29-6.82h.02Z"/>
            </svg>
          </span>
        ))}
      </div>
    </div>
  );
}
